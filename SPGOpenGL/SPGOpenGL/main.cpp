...
glVertex3f */
//1 vertex format din 3 valori(x,y,z) de tip float 
...
void draw(void)
{
	...
	glShadeModel(GL_SMOOTH);// glShedeModel determina ce culoarea va fi interpolata intre vertexuri
							//GL_Smooth se refera strict la luminozitate
	glBegin(GL_POLYGON);
	glColor3f(10.0, 20.0, 0.0); //partea superioara a ecranului
	v3f(-20, 20, -19);
	v3f(20, 20, -19);
	glColor3f(0.0, 200.0, 1.0); // partea inferioara a ecranului
	v3f(20, -20, -19);
	v3f(-20, -20, -19);
	glEnd();
	...
	glShadeModel(GL_FLAT);  // Flat culoarea vertexului este asignata toturor pixelilor din fragment : https://www.khronos.org/registry/OpenGL-Refpages/gl2.1/xhtml/glShadeModel.xml
	...
	glutSwapBuffers(); //schimba bufferul ferestrei daca este dublu bufferat.
}

// logica deplasarii elementelor
...
main(int argc, char *argv[])
{
	...
	//GLUT_DOUBLE - adauga o fereastra dublu buffer-ata
	//GLUT_RGB - Bit mask to select an RGBA mode window.
	//GLUT_DEPTH - Bit mask to select a window with a depth buffer.
	//GLUT_MULTISAMPLE - Bit mask to select a window with multisampling support; client-side and server-side
